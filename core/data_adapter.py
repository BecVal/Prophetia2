...
def process_match(row):
    ...
    if not os.path.exists(file_path):
        logger.warning(
            f"Archivo de eventos no encontrado para el partido {match_id}")
        return []
...
def build_intermediate_from_statsbomb():
    if not os.path.exists(MATCHES_METADATA_PATH):
        logger.error(
            f"No se encontró el archivo de partidos en: {MATCHES_METADATA_PATH}")
        return

    matches_df = pd.read_parquet(MATCHES_METADATA_PATH, engine='fastparquet')
    logger.info(
        f"Generando dataset intermedio a partir de {len(matches_df)} partidos de StatsBomb...")
    ...
    logger.info(
        f"Dataset intermedio universal guardado exitosamente en: {INTERMEDIATE_OUTPUT_PATH}")
    logger.info(
        f"Estructura del dataset intermedio (filas = 2x partidos originales): {final_df.shape}")

if __name__ == '__main__':
    script_dir = os.path.dirname(os.path.abspath(__file__))
    os.chdir(script_dir)
    build_intermediate_from_statsbomb()
